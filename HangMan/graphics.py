...
class _BBox(GraphicsObject):
    ...
    def getCenter(self):
        p1 = self.p1
        p2 = self.p2
        return p((p1.x+p2.x)/2.0, (p1.y+p2.y)/2.0)
...
class Circle(Oval):
    ...
    def __init__(self, center, radius):
        p1 = p(center.x-radius, center.y-radius)
        p2 = p(center.x+radius, center.y+radius)
...
class Polygon(GraphicsObject):
    ...
    def __init__(self, *points):
        ...
        if len(points) == 1 and type(points[0]) == type([]):
            points = points[0]
        self.points = list(map(p.clone, points))
    ...
    def getPoints(self):
        return list(map(p.clone, self.points))
...
def test():
    ...
    t = Text(p(5,5), "Centered Text")
    t.draw(win)
    p = Polygon(p(1,1), p(5,3), p(2,7))
    p.draw(win)
    e = Entry(p(5,6), 10)
    e.draw(win)
    win.getMouse()
